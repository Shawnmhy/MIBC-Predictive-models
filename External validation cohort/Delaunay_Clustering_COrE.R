...
###################################################
HE_ptsFile <- read.csv('./HE_Coordinates_Features.csv')

HE_metaFile <- read.csv('TMA_HE_Meta.csv')

#----------------------------------#
for(coreName in CORE.VALID$Name){
  ...
  if(nrow(ptsFile) != 0){
    ...
    ref_x <- HE_metaFile[HE_metaFile$Name == coreName,]$Centroid.X.µm - 4340*0.2305/2 # unit: pixel
    ref_y <- HE_metaFile[HE_metaFile$Name == coreName,]$Centroid.Y.µm - 4340*0.2305/2 # unit: pixel
    
    # get the rescaled points
    rescale_x <- ptsFile$Centroid.X.µm - ref_x
    rescale_y <- ptsFile$Centroid.Y.µm - ref_y  
    
    HE_ptsFile[HE_ptsFile$TMA.core == coreName, 3:4] <- cbind(rescale_x, rescale_y) 
  }
}
...
TMA_allArea <- read.csv('./TMA_allArea_combined.csv', row.names = 1)
...
for(core in unique(CORE.VALID$Name)){
  
  #core <- 'A-1'

  # reference
  Region_HE <- readRDS(paste('./TMA_Boundary_Rescaled/', core, '/HE.rds', sep = ''))
  #Region_HE <- lapply(Region_HE,FUN= function(x) x*0.454)
  
  # pts to ppp
  pts <- data.frame(HE_ptsFile[HE_ptsFile$TMA.core == core, 3:4])
  ...
  if(length(cluster_posListAll) != 0){
    ...
    tissueArea <- TMA_allArea[TMA_allArea$core == core,2]

...
source('~/Desktop/Validation-cohort/FeatureFunction.R')


HE_detection <- read.table('./HE_Coordinates_Features.csv',sep = ',' , header = TRUE)
...
# get core meta file
coreMetaFile <- read.csv('TMA_HE_Meta.csv')
...
for(coreName in coreMetaFile$Name){
  #coreName <- 'A-1'
  ...
  if(category == 'False'){
    ...
    cellShape_list <- read.csv(paste('./HE_Nucleus_Centroids_combined/detection_coordinate_',coreName, '.csv', sep = ''), row.names = 1)
    
    
    #ggplot() +
    #  geom_polygon(data = cellShape_list, aes(x = Centroid.X.µm, y = Centroid.Y.µm, group = Cell.ID))+
    #  geom_point(data = subDF,aes(x = subDF$Centroid.X.µm, y = subDF$Centroid.Y.µm), color = 'red')
    ...
    shapeFeature_all <- rbind(shapeFeature_all, cbind(coreName, areaStat, lengthStat, circStat, 
                                                      ...
                                                      curvStdStat, n_protrusion_Stat, n_indentation_Stat, MOIStat))
  }
  print(coreName)
}
...
#------------- plot single cell ------------@
cell.contr <- cellShape_list[cellShape_list$Cell.ID == '108',]


ggplot(data  = cell.contr) +
  geom_polygon(aes(x = cell.contr$Centroid.X.µm, y = cell.contr$Centroid.Y.µm), fill = NA, color = 'black')+
  coord_equal()

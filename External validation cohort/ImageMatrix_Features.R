...
qualifiedCore <- read.csv('./CORE.VALID.csv')['Name']
...
for(coreName in coreMetaFile$Name){
  ...
  #coreName <- 'A-1'
  ## tiles
  tilePath <- paste('./Tiles/', coreName, sep ='')
  ## coordiante file
  ...
  if(category == 'False'){
    ...
    Response <- as.character(coreMetaFile[coreMetaFile$Name == coreName,]$Response)
    
    coreStats <- rbind(coreStats, cbind(coreName, patientID, Response))

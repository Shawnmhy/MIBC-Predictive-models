...
getCurvature <- function(xy, ptsCount) {
  ...
  return(cbind(curvMean, curvMin, curvMax, curvStd, n_protrusion, n_indentation))
}
...
getCOrE <- function(cluster_posListAll, w){
  
  COrE_features.All <- matrix(nrow = 0, ncol = 13)
  #w <- 18
  cluster_posListAll <- data.frame(cluster_posListAll)
  
  cluster_posListAll$cluster <- as.numeric(as.character(cluster_posListAll$cluster))
  # angle diretization
  N <- 360/w
  # define a small positive number to avoid log error
  eps <- 10^(-15)
  cluster_posListAll$orientation <- ceiling((cluster_posListAll$orientation)/w)*w
  
  # create co-occurence matrix
  co_occurrence <- matrix(0, nrow = N, ncol = N)
  for(clus_id in 1:max(cluster_posListAll$cluster)){
    
    co_occurrence <- matrix(0, nrow = N, ncol = N)
    
    neighborhood <- cluster_posListAll[cluster_posListAll$cluster == clus_id,]
    for(col in 1:N){
      for(row in 1:N){
        
        if(col == row){
          co_occurrence[row, col] <- co_occurrence[row, col] + as.numeric(choose(nrow(neighborhood[neighborhood$orientation == col*w,]),2))
        } else {
          co_occurrence[row, col] <- co_occurrence[row, col] + as.numeric(nrow(neighborhood[neighborhood$orientation == col*w,])*
                                                                            nrow(neighborhood[neighborhood$orientation == row*w,]))
        }
    ...
    COrE_features.All <- rbind(COrE_features.All, COrE_features)
  }
  
  # get feature names
  COrE.Fname <- colnames(COrE_features.All)
  
  
  # name mean features
  COrE_features.mean <- data.frame(t(colMeans(COrE_features.All)))
  Name1 <- as.vector(t(outer(COrE.Fname, '_mean', paste, sep = '')))
  colnames(COrE_features.mean) <- Name1
  
  
  COrE_features.max <- data.frame(t(colMaxs(data.matrix(COrE_features.All))))
  Name2 <- as.vector(t(outer(COrE.Fname, '_max', paste, sep = '')))
  colnames(COrE_features.max) <- Name2
  
  COrE_features.min <- data.frame(t(colMins(data.matrix(COrE_features.All))))
  Name3 <- as.vector(t(outer(COrE.Fname, '_min', paste, sep = '')))
  colnames(COrE_features.min) <- Name3

  
  
  COrE_features.final <- cbind(COrE_features.mean, COrE_features.max, COrE_features.min)
  return(COrE_features.final)

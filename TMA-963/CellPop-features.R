...
CORE.963 <- read.csv('./TMA-963-Annotations/qualified_TMAcores.csv', row.names = 1)
CORE.1042 <- read.csv('~/Desktop/TMA-coordinates/qualified_TMAcores.csv', row.names = 1)

CORE.963.area <- read.csv('TMA_allArea.csv', row.names = 1)
CORE.1042.area <- read.csv('~/Desktop/TMA-coordinates/TMA_allArea.csv', row.names = 1)

Cell.963.label <- read.csv('Cell.label.csv')
Cell.1042.label <- read.csv('~/Desktop/TMA-coordinates/Cell.label.csv')

#---------------------------------------#
# Rescale H&E points for 963 -----------#

TMA.963.meta <- read.csv('TMA-HE-Meta-CellClassifier.csv')
...
for(core in CORE.963$x){
  ...
  ptsFile <- Cell.963.label[Cell.963.label$TMA.core == core, c(3:5)] 
  
  # get ref points
  
  ref_x <- TMA.963.meta[TMA.963.meta$Name == core,]$Centroid.X.µm - 2908*0.454/2 # unit: pixel
  ref_y <- TMA.963.meta[TMA.963.meta$Name == core,]$Centroid.Y.µm - 2908*0.454/2 # unit: pixel
  ...
  # get type
  Cell.963.label[Cell.963.label$TMA.core == core, 3:4] <- cbind(rescale_x, rescale_y) 
...
for(core in CORE.1042$x){
  ...
  # subset DF, convert to pixel unit
  ptsFile <- Cell.1042.label[Cell.1042.label$TMA.core == core, c(3:5)] 
  
  # get ref points
  
  ref_x <- TMA.1042.meta[TMA.1042.meta$Name == core,]$Centroid.X.µm - 3030*0.454/2 # unit: pixel
  ref_y <- TMA.1042.meta[TMA.1042.meta$Name == core,]$Centroid.Y.µm - 3030*0.454/2 # unit: pixel
  ...
  # get type
  Cell.1042.label[Cell.1042.label$TMA.core == core, 3:4] <- cbind(rescale_x, rescale_y) 
  
}


# prepare tables for python
#write.csv(Cell.1042.label, '~/Desktop/TMA_DoC/Cell.1042.label.csv')
...
Mutual.dist <- data.frame(matrix(nrow = 0, ncol = 51))

Cell.1042.label <- read.csv('~/Desktop/TMA_DoC/Cell.1042.label.csv')
...
Region.963.path <- '~/Desktop/TMA_963/TMA - Coordinates/TMA_SpatStat_Rescaled/'
Region.1042.path <- '~/Desktop/TMA - coregistration/TMA_SpatStat_Rescaled/'

for(TMA.set in c(963,1042)){
  
  #TMA.set <- '963'
  CORE <- eval(sym(paste('CORE.', TMA.set, sep = '')))
  
  CORE.area <- eval(sym(paste('CORE.', TMA.set, '.area', sep = '')))
  
  Cell.label <- eval(sym(paste('Cell.', TMA.set, '.label', sep = '')))
  
  Region.path <- eval(sym(paste('Region.', TMA.set, '.path', sep = '')))
  
  for(core in CORE$x){
    
    #core <- 'A-9'
    core.area <- CORE.area[CORE.area$TMA.core == core, ]$H.E
    ...
    # composite
    Mutual.dist <- rbind(Mutual.dist, cbind(core, TMA.set, list1[[1]], list1[[2]],  list2[[1]], list2[[2]], list3[[1]], list3[[2]],cancer.ratio, stromal.ratio, lympho.ratio,
...
colnames(Mutual.dist)[3:11] <- c('Kcross.Cancer2lympho', 'Kcross.lympho2Cancer', 'Kcross.Cancer2Stromal', 'Kcross.Stromal2Cancer', 'Kcross.Lympho2Stromal', 'Kcross.Stromal2Lympho', 'cancer ratio', 'stromal ratio', 'lymphocytes ratio')
...
# read ternary data
ternary.dat <- Mutual.dist[c('core', 'TMA.set', 'cancer ratio', 'stromal ratio', 'lymphocytes ratio')]

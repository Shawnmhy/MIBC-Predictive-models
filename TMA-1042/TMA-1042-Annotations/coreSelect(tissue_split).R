...
heatmap.matrix <- matrix(nrow = 12, ncol = 13)
...
for(letter in LETTERS[1:12]){
  ...
  for(num in 1:13){
    ...
    vec <- as.numeric(as.character(Merge_stats[Merge_stats$TMA.core == coreName, 2:11]))
    heatmap.matrix[row, num] <- sd(vec)/mean(vec)
    
  }
}
write.csv(heatmap.matrix, '1042.heatmapMatrix.csv')
...
write.csv(CK56_part_stat, '1042_CK56_part.csv')


# ------ inter and intra correlation evaluation -------- #
{


# this section measures inter-pair pearson of CK5-6 to others:
sum <- 0
for(id in 2:6){
  
  # HE
  a <- cor.test(CK56_part_stat[,id], HE_part_stat[,2],  method = "spearman")$estimate
  b <- cor.test(CK56_part_stat[,id], HE_part_stat[,3],  method = "spearman")$estimate
  c <- cor.test(CK56_part_stat[,id], HE_part_stat[,4],  method = "spearman")$estimate

  
  # Ki67
  d <- cor.test(CK56_part_stat[,id], Ki67_part_stat[,2],  method = "spearman")$estimate
  e <- cor.test(CK56_part_stat[,id], Ki67_part_stat[,3],  method = "spearman")$estimate

  sum = sum + (a + b + c + d + e)
  
}

# this section measures inter-pair pearson of Ki-67 to others:

for(id in 2:3){
  
  # HE
  a <- cor.test(Ki67_part_stat[,id], HE_part_stat[,2],  method = "spearman")$estimate
  b <- cor.test(Ki67_part_stat[,id], HE_part_stat[,3],  method = "spearman")$estimate
  c <- cor.test(Ki67_part_stat[,id], HE_part_stat[,4],  method = "spearman")$estimate
...
sum <- 0
# this section measures inter-pair pearson of CK56 to others:

a <- cor.test(CK56_part_stat[,2], CK56_part_stat[,3],  method = "spearman")$estimate
b <- cor.test(CK56_part_stat[,2], CK56_part_stat[,4],  method = "spearman")$estimate
c <- cor.test(CK56_part_stat[,2], CK56_part_stat[,5],  method = "spearman")$estimate
d <- cor.test(CK56_part_stat[,2], CK56_part_stat[,6],  method = "spearman")$estimate
e <- cor.test(CK56_part_stat[,3], CK56_part_stat[,4],  method = "spearman")$estimate
f <- cor.test(CK56_part_stat[,3], CK56_part_stat[,5],  method = "spearman")$estimate
g <- cor.test(CK56_part_stat[,3], CK56_part_stat[,6],  method = "spearman")$estimate
h <- cor.test(CK56_part_stat[,4], CK56_part_stat[,5],  method = "spearman")$estimate
i <- cor.test(CK56_part_stat[,4], CK56_part_stat[,6],  method = "spearman")$estimate
j <- cor.test(CK56_part_stat[,5], CK56_part_stat[,6],  method = "spearman")$estimate
...
a <- cor.test(Ki67_part_stat[,2], Ki67_part_stat[,3],  method = "spearman")$estimate


sum = sum + (a )


# this section measures inter-pair pearson of HE to others:


a <- cor.test(HE_part_stat[,2], HE_part_stat[,3],  method = "spearman")$estimate
b <- cor.test(HE_part_stat[,2], HE_part_stat[,4],  method = "spearman")$estimate
c <- cor.test(HE_part_stat[,3], HE_part_stat[,4],  method = "spearman")$estimate
...
cor.test(CK56_part_stat[,2], CK56_part_stat[,6],  method = "pearson")

dat <- data.frame(cbind(CK56_part_stat[,5], Ki67_part_stat[,3]))
ggplot(data = dat, aes(CK56_part_stat[,5], Ki67_part_stat[,3])) +
  geom_point(shape = 21, colour = 'black', fill = '#F48A9A', size = 8) + #8AD5F4: blue
  geom_smooth(method = "lm", se = FALSE, color = 'black') +
  xlab('GATA3 (Cluster 1)') +
  ylab('Cyclin (Cluster 3)') +
  theme(axis.ticks = element_blank(), axis.text = element_blank(), axis.title = element_text(size = 25)) 
